...
class DethemeKit_Template_Library{

    const TRANSIENT_KEY = 'dethemekit_template_info';

    public static $endpoint = 'https://dethemekit.com/library/wp-json/dethemekit/v1/templates';
    public static $templateapi = 'https://dethemekit.com/library/wp-json/dethemekit/v1/templates/%s';
    ...
    function __construct(){
        if ( is_admin() ) {
            ...
            add_action( 'wp_ajax_dethemekit_ajax_request', [ $this, 'templates_ajax_request' ] );

            add_action( 'wp_ajax_dethemekit_ajax_get_required_plugin', [ $this, 'ajax_plugin_data' ] );
            add_action( 'wp_ajax_dethemekit_ajax_plugin_activation', [ $this, 'ajax_plugin_activation' ] );
            add_action( 'wp_ajax_dethemekit_ajax_theme_activation', [ $this, 'ajax_theme_activation' ] );
        }
        
        add_action( 'admin_enqueue_scripts', [ $this, 'scripts' ] );

        self::$api_args = [
            'plugin_version' => DETHEMEKIT_ADDONS_VERSION,
            'url'            => home_url(),
        ];
    ...
    public static function get_api_endpoint(){
        if( is_plugin_active('dethemekit-addons-pro/dethemekit_addons_pro.php') && function_exists('dethemekit_pro_template_endpoint') ){
            self::$endpoint = dethemekit_pro_template_endpoint();
        }
    ...
    public static function get_api_templateapi(){
        if( is_plugin_active('dethemekit-addons-pro/dethemekit_addons_pro.php') && function_exists('dethemekit_pro_template_url') ){
            self::$templateapi = dethemekit_pro_template_url();
        }
    ...
    public function admin_menu() {
        add_submenu_page(
            'dethemekit_page', 
            esc_html__( 'Template Library', 'dethemekit' ),
            esc_html__( 'Template Library', 'dethemekit' ), 
            'manage_options', 
    ...
    public function library_render_html(){
        require_once DETHEMEKIT_ADDONS_PATH . 'includes/admin/include/templates_list.php';
    }
    ...
    public function scripts( $hook ) {
        ...
        if( 'dethemekit_page_dethemekit_templates' == $hook ){

            // CSS
            wp_enqueue_style( 'dethemekit-selectric' );
            wp_enqueue_style( 'dethemekit-temlibray-style' );

            // JS
            wp_enqueue_script( 'dethemekit-modernizr' );
            wp_enqueue_script( 'jquery-selectric' );
            wp_enqueue_script( 'jquery-ScrollMagic' );
            wp_enqueue_script( 'babel-min' );
            wp_enqueue_script( 'dethemekit-templates' );
            wp_enqueue_script( 'dethemekit-install-manager' );
        ...
        foreach ( $plugins as $key => $plugin ) {
            ...
            if ( ! is_wp_error( $data ) ) {
                ...
                if ( file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) && is_plugin_inactive( $data['location'] ) ) {
                    ...
                    $button_text    = esc_html__( 'Activate', 'dethemekit' );

                // Not Installed.
                } elseif ( ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {

                    $button_classes = 'button install-now';
                    $button_text    = esc_html__( 'Install Now', 'dethemekit' );

                // Active.
                } else {
                    $button_classes = 'button disabled';
                    $button_text    = esc_html__( 'Activated', 'dethemekit' );
                }
                    ...
                    <li class="htwptemplata-plugin-<?php echo $data['slug']; ?>">
                        ...
                        <?php
                            if ( $type == 'pro' && ! file_exists( WP_PLUGIN_DIR . '/' . $data['location'] ) ) {
                                echo '<a class="button" href="'.esc_url( $data['pllink'] ).'" target="_blank">'.esc_html__( 'Buy Now', 'dethemekit' ).'</a>';
                            }else{
                        ?>
                    ...
                    <li class="htwptemplata-theme-<?php echo $data['slug']; ?>">
                        ...
                        <?php
                            if ( !empty( $data['prolink'] ) ) {
                                echo '<a class="button" href="'.esc_url( $data['prolink'] ).'" target="_blank">'.esc_html__( 'Buy Now', 'dethemekit' ).'</a>';
                            }else{
                        ?>
    ...
    public function ajax_plugin_activation() {
        ...
        if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['location'] ) || ! $_POST['location'] ) {
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Plugin Not Found', 'dethemekit' ),
                )
            );
        }
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => esc_html__( 'Plugin Successfully Activated', 'dethemekit' ),
            )
        );
    ...
    public function ajax_theme_activation() {
        ...
        if ( ! current_user_can( 'install_themes' ) || ! isset( $_POST['themeslug'] ) || ! $_POST['themeslug'] ) {
            wp_send_json_error(
                array(
                    'success' => false,
                    'message' => esc_html__( 'Sorry, you are not allowed to install themes on this site.', 'dethemekit' ),
                )
            );
        }
        ...
        wp_send_json_success(
            array(
                'success' => true,
                'message' => __( 'Theme Activated', 'dethemekit' ),
            )
        );
    }
...
DethemeKit_Template_Library::instance();