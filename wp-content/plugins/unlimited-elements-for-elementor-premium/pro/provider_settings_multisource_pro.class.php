...
class UniteCreatorSettingsMultisourcePro{
	...
	public function addItemsMultisourceSettings($name, $value, $title, $param){
		...
		//third party plugin api's
		
		$isApiEnabled = UniteCreatorAPIIntegrations::isEnabled();
	...
	private function addMultisourceConnectors_api($name, $arrIncludedAttributes){
		...
		$arrApiTypes = apply_filters(UniteCreatorAPIIntegrations::HOOK_GET_TYPES, $arrApiTypes);
		
				
		$condition = array($name."_source"=>self::TYPE_API);
		
		
		//add empty text if no api's found
		
		if(empty($arrApiTypes)){
		...
		$arrApiTypes = array_flip($arrApiTypes);
		
		$arrApiTypes = UniteFunctionsUC::addArrFirstValue($arrApiTypes, "", "[Select API Type]");
		
		$this->settings->addSelect($name."_api_type", $arrApiTypes, $text, "", $params);
